...
quote_processing_prompt_template_ = """
    Fill the quotation template with the given data. Here is the mapping between JSON fields and HTML elements:
    {{
...
def quote_pdf_agent_node(state: agentState):
    print("[INVOKE][quote_pdf_agent_node]")
    ...
    quotation_html_template_path = f"/Users/keven/Desktop/product_v01/quotation_html_template.html"
    # Read the HTML template using absolute path
    with open(quotation_html_template_path, 'r') as f:
        html_src = f.read()

    system_prompt = quote_processing_prompt_template_.format(
    ...
    # Save HTML first
    html_path = save_quotation_html(filled_html, state.quotation_json, output_dir)
    
    # Save PDF using the savePDFToFile function
    pdf_filename = f"_quotation_pdf_{state.quotation_json['code_no']}.pdf"
    pdf_path = os.path.join(output_dir, pdf_filename)
    
    return {
        "messages": [AIMessage(content=f"Quotation generated. Files saved:\nHTML: {html_path}\nPDF: {pdf_path}")]
    }
